...
```json
{
  ...
  "with_evidence_samples": [{"sample_index": 0, "extracted_answer": "string", "correct": true}],
  "question_only_samples": [{"sample_index": 0, "extracted_answer": "string", "correct": false}],
  "passed": true,
  "reason": "string"
}
```
...
1. 使用仓库现有 model client 调用 verifier solver 两组各 3 次：一组输入完整 evidence bundle 和问题，另一组仅输入问题；两组均禁用搜索和工具。
2. 使用与 solver reward 相同的归一化精确匹配比较每次提取答案与 `reference_answer`，不调用独立 judge。
3. 仅当有 evidence 的 3 次中至少 1 次正确、且仅问题的 3 次全部错误时设置 `passed=true`。
4. `passed=true` 的样本保留；两条件不成立时标记 `verify_failed`，调用故障时标记 `verify_error`，均不进入本轮 generated data。
...
1. verify 仅在“有 evidence 至少一次正确 + 仅问题三次全错”时通过。
2. verifier 单题调用失败和解析失败均过滤该样本并继续后续 verify。
